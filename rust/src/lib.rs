...
async fn monitor_lightning_receive(wallet: Wallet, tx: Sender<CoreMsg>, payment_hash: PaymentHash) {
    let payment_hash_text = payment_hash.to_string();
...
async fn upload_profile_picture(keys: Keys, image_base64: String) -> anyhow::Result<String> {
...
fn metadata_from_state(nostr: &NostrState) -> anyhow::Result<Metadata> {
...
fn public_key_from_npub_or_hex(value: &str) -> anyhow::Result<NostrPublicKey> {
...
fn merge_contacts(existing: &mut Vec<Contact>, fetched: Vec<Contact>) {
